...
!
! ifort g_radm_2018_CF.f90 -o radm2018CF.exe -O2 -axAVX -lnetcdff -L$NETCDF/lib -I$NETCDF/include
!
!
...
character(len=38),dimension(radm):: cname=(/'carbon_monoxide','ammonia','nitrogen_monoxide', &
'nitrogen_dioxide','sulfur_dioxide','aldehydes','methane','aromatic_compounds','alkenes','aldehydes', &
'alkanes','alkanes','alkanes','formaldehyde','isoprene','acetone','aldehydes', &
'aldehydes','ketones','alkenes','alkenes','alkynes','organic_acids','organic_acids', &
'toluene','xylene' ,'carbon_dioxide','pm10_dry_aerosol_particles', &
'pm2p5_dry_aerosol_particles','sulfate_dry_aerosol_particles','nitrate_dry_aerosol_particles',&
'pm2p5_dry_aerosol_particles','organic_carbon_dry_aerosol_particles ','elemental_carbon_dry_aerosol_particles',&
'sulfate_dry_aerosol_particles','nitrate_dry_aerosol_particles','pm2p5_dry_aerosol_particles',&
'organic_carbon_dry_aerosol_particles','elemental_carbon_dry_aerosol_particles'/)
...
!       PM10  PM2.5  PSO4 PNO3 OTHER POA   PEC  CH4   CN
  ...
  allocate(eft(nx,ny,nf,nh,1)) !one level
  zlev=0
  eft=0
  do k=1,ncel
    ...
    busca: do j=1,ny
      do i=1,nx
        ...
        if(idcg(k).eq.idcf) then
          do ih=1,nh
            eft(i,j,ii,ih,1)=eft(i,j,ii,ih,1)+edum(ih)/3.6e12*scala(ii) ! Emission from kg to gmol
          end do
          exit busca
        end if
      end do
		  ...
		  do i=1,nx
			...
			if(idcg(k).eq.idcf) then
			  do ih=1,nh
                ...
                eft(i,j,ii,ih,1)=eft(i,j,ii,ih,1)+edum(ih)/3.6e12*scalm(ii)
			  end do
			  exit busca2
			end if
		 end do
		 ...
		 busca3: do j=1,ny
		  ...
		  do ih=1,nh
                ...
                if(ih.gt.9 .and. ih.lt.19) then
                  if(levl.lt.2) then
                    eft(i,j,ii,ih,1)=eft(i,j,ii,ih,1)+edum(ih)/3.6e12*scalp(ii)
                   else
                    eft(i,j,ii,ih,1)=eft(i,j,ii,ih,1)+edum(ih)/3.6e12
                  end if
                 else
                  if(levld.lt.2) then
                    eft(i,j,ii,ih,1)=eft(i,j,ii,ih,1)+edum(ih)/3.6e12*scalp(ii)
...
!   CCC R   R EEEEE A   A____A   A   T     T   R   R
	  ...
	  character(len=96) :: cvar
     cvar="tendency_of_atmosphere_mass_content_of_"//trim(cname)//"_due_to_emission"
	   call check( nf90_def_var(ncid, svar, NF90_REAL, dimids,id_var ) )
 ! Assign  attributes
        call check( nf90_put_att(ncid, id_var, "_FillValue", 1e20 ) )
        call check( nf90_put_att(ncid, id_var, "long_name", trim(cname)) )
        call check( nf90_put_att(ncid, id_var, "standard_name", Cvar) )
        call check( nf90_put_att(ncid, id_var, "units", "kg m-2 s-1"))
...
!   CCC R   R EEEEE A   A____A   A   T     T   R   R 22222
     ...
     cvar="tendency_of_atmosphere_mass_content_of_"//trim(cname)//"_due_to_emission"
	   call check( nf90_def_var(ncid, svar, NF90_REAL, dimids,id_var ) )
 ! Assign  attributes
        call check( nf90_put_att(ncid, id_var, "FieldType", 1e20) )
        call check( nf90_put_att(ncid, id_var, "long_name",trim(cname)) )
        call check( nf90_put_att(ncid, id_var, "standard_name",cvar) )
        call check( nf90_put_att(ncid, id_var, "units", "kg m-2 s-1"))
		call check( nf90_put_att(ncid, id_var, "coordinates", "XLONG XLAT") )
