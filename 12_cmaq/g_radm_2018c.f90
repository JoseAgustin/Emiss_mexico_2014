...
!
! ifort -O2 -axAVX -lnetcdff -L$NETCDF/lib -I$NETCDF/include g_radm_2018c.f90 -o radm2018c.exe
!
!
...
module vars
	...
	'NO2  ','SO2  ','ALD2 ','CH4  ','CSL  ','ETH  ','GLY  ', &
	'HC3  ','HC5  ','HC8  ','FORM ','ISOP ','KET  ','MACR ', &
	'MGLY ','MVK  ','OL2  ','OLI  ','OLT  ','ORA1 ','ORA2 ', &
	'TOL  ','XYL  ','CO2  ','PM_10','FPRM ','PSO4 ','PNO3 ','PM25I',&
	'POA  ','PEC  '/)
    character(len=16),dimension(radm):: cname=(/'Carbon Monoxide ','NH3             ','NO              ', &
	'NO2  ','SO2  ','ALDEHYDES  ','METHANE','CRESOL','Ethane','Glyoxal', &
	'HC3  ','HC5  ','HC8  ','FORM ','ISOPRENE','Acetone','Acrolein', &
...
! Mole weight
  ...
  &    42., 46., 60., 92.,106.,44.,&
  &   1.,1.,1.,1.,1.,1.,1./! MW 3600 for unit conversion to ug/s
...
subroutine store
    ...
    integer :: id,iu
    integer :: anio,mes,dia,julday
    integer :: isp(radm)
    integer :: ihora,imin,isec,icdate,ictime
    ...
    character(8)  :: date
    character(10) :: time
    character(24) :: hoy !ihora,imin,isec
...
! ******************************************************************
    call date_and_time(date,time)
     hoy=date(7:8)//'-'//date(5:6)//'-'//date(1:4)//' '//time(1:2)//':'//time(3:4)//':'//time(5:10)
...
call check( nf90_put_att(ncid, NF90_GLOBAL, "IOAPI_VERSION","$Id: @(#) ioapi library version 3.0 OpenMP enabled $"))
      ...
      call check( nf90_put_att(ncid, NF90_GLOBAL, "VAR-LIST ","CO              NH3             NO              NO2             SO2             ALD2            CH4             CSL             ETH             GLY             HC3             HC5             HC8             FORM            ISOP            KET             MACR            MGLY            MVK             OL2             OLI             OLT             ORA             ORA2            TOL             XYL             CO2             PM_10           FPRM            PSO4            PNO3            PM25I           POA             PEC             SO4I            NO3J            PM25J           ORGJ            ECJ             "))
...
!              call check( nf90_put_var(ncid, id_varpop,pob,start=(/1,1,it-11/)) )
        ...
        do i=1, nx
          do j=1, ny
            do l=1,zlev
              ea(i,j,l,1)=eft(i,j,ikk,it+1,l)  !entre 9x9 km
            end do
          end do
        end do
!
